...
def get_alerts(nurse_id: int):
    try:
        r = requests.get(f"{API_BASE}/alerts", params={"user_type": "nurse", "user_id": nurse_id}, timeout=10)
...
def put_alert_ack(alert_id: int, nurse_id: int):
    try:
        r = requests.put(f"{API_BASE}/alerts/{alert_id}", json={"user_type": "nurse", "user_id": nurse_id}, timeout=10)
        if r.status_code != 200:
